...
Usage for AI Agents:
    from forge_base.dev.api import QualityChecker, ScaffoldGenerator
...
def get_agent_quickstart() -> str:
    ...
    try:
        ...
        if hasattr(resources, 'files'):
            # Python 3.9+
            doc_file = resources.files('forge_base._docs') / 'AI_AGENT_QUICK_START.md'
        ...
        # Fallback: read from docs/ai-agents/ if in development
        project_root = Path(__file__).parent.parent.parent.parent
        quickstart_path = project_root / 'docs' / 'ai-agents' / 'quick-start.md'
        ...
        return (
            ...
            "Please visit: https://github.com/symlabs-ai/forge_base\n"
        )
...
def get_documentation_path() -> Path:
    ...
    try:
        if hasattr(resources, 'files'):
            # Python 3.9+
            return Path(str(resources.files('forge_base'))) / 'docs'
